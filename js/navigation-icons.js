/**
 * Replace [data-lucide] placeholders in #app-navigation with inline SVGs (CSP-friendly).
 * Loaded as an app script (no inline nonce); must stay a direct sibling flow under #app-navigation
 * in the template — only the <script> tag is external now.
 *
...
(function () {
	...
	var navSvgIcons = {
		...
		shield: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="lucide-icon" aria-hidden="true"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>'
	};

	function fillNavIcons() {
		var list = document.querySelectorAll('#app-navigation [data-lucide]');
		for (var i = 0; i < list.length; i++) {
			var el = list[i];
			var n = el.getAttribute('data-lucide');
			if (n && navSvgIcons[n]) {
				el.innerHTML = navSvgIcons[n];
			}
		}
	}

	if (document.readyState === 'loading') {
		document.addEventListener('DOMContentLoaded', fillNavIcons);
	} else {
		fillNavIcons();
	}
}());
