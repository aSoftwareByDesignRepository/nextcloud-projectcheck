...
const ProjectControlValidation = {
  ...
  setupCustomValidators() {
    // Add project-specific validators here
    this.addRule('uniqueProjectName', {
      test: async (value, field, form) => {
        // This would typically make an AJAX call to check uniqueness
        // For now, we'll simulate it
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve(value !== 'existing-project');
          }, 100);
        });
      },
    ...
    // Enhanced time entry validators
    this.addRule('timeEntryOverlap', {
      test: async (value, field, form) => {
        // Check for time entry overlaps within the same day
        const dateField = form.querySelector('[name="date"]');
        const startTimeField = form.querySelector('[name="start_time"]');
        const endTimeField = form.querySelector('[name="end_time"]');
        ...
        try {
          const response = await fetch(OC.generateUrl('/apps/projectcheck/time-entries/check-daily-overlap'), {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'requesttoken': OC.requestToken
          ...
          console.error('Error checking daily time overlap:', error);
          return true;
        }
      },
      message: 'This time entry overlaps with another entry on the same day'
    });

    // Project assignment validators
    this.addRule('userAvailability', {
      test: async (value, field, form) => {
        // Check if user is available for the project assignment
        const userId = field.getAttribute('data-user-id') || OC.getCurrentUser().uid;
        const projectId = value;
        ...
        try {
          const response = await fetch(OC.generateUrl('/apps/projectcheck/users/' + userId + '/availability'), {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'requesttoken': OC.requestToken
      ...
      message: 'User is not available for this project assignment'
    });

    // Budget validation
    this.addRule('projectBudgetLimit', {
      test: async (value, field, form) => {
        // Check if time entry would exceed project budget
        const hours = parseFloat(value);
        const projectId = form.querySelector('[name="project_id"]')?.value;
        const hourlyRate = parseFloat(form.querySelector('[name="hourly_rate"]')?.value || 0);
        
        if (!projectId || isNaN(hours) || isNaN(hourlyRate)) {
          return true;
        }
        ...
        try {
          const response = await fetch(OC.generateUrl('/apps/projectcheck/projects/' + projectId + '/budget-status'), {
            headers: {
              'requesttoken': OC.requestToken
            }
          });
          ...
          const result = await response.json();
          const remainingBudget = result.totalBudget - result.spentBudget;
      ...
      message: 'This time entry would exceed the remaining project budget'
    });

    // Customer validation
    this.addRule('customerProjectLimit', {
      test: async (value, field, form) => {
        // Check if customer has reached their project limit
        const customerId = value;
        ...
        try {
          const response = await fetch(OC.generateUrl('/apps/projectcheck/customers/' + customerId + '/project-limit'), {
            headers: {
              'requesttoken': OC.requestToken
            }
          });
          ...
          const result = await response.json();
          return result.canCreateProject;
        } catch (error) {
          console.error('Error checking customer project limit:', error);
          return true;
        }
      },
      message: 'This customer has reached their maximum number of active projects'
    });
  },
