 ...
 * @var \OCP\IL10N $l
 * @var \OCA\ProjectCheck\Support\AppFeedbackLinks $appFeedbackLinks optional; constructed when omitted
...
if (!isset($appFeedbackLinks) || !$appFeedbackLinks instanceof AppFeedbackLinks) {
	$appFeedbackLinks = new AppFeedbackLinks('projectcheck', 'ProjectCheck', $version);
}
$pageUrl = '';
...
>
	<div class="<?php p($prefix); ?>-nav-footer__popover">
		<button
			type="button"
			class="<?php p($prefix); ?>-nav-footer__trigger pc-nav__link"
			aria-expanded="false"
			aria-controls="<?php p($menuId); ?>"
			aria-haspopup="true"
		>
			<span class="pc-nav__icon" aria-hidden="true"><?php
				print_unescaped(IconCatalog::render('info'));
			?></span>
			<span class="pc-nav__label">
				<span class="pc-nav__name"><?php p($l->t('Help & Feedback')); ?></span>
			</span>
		</button>
		<ul
