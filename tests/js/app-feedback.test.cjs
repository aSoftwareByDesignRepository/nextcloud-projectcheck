#!/usr/bin/env node
/**
 * Behavioural tests for ProjectCheck app-feedback.js (mailto + toast hook).
 *
...
function boot(extra) {
	...
	const config = {
		appId: 'projectcheck',
		appDisplayName: 'ProjectCheck',
		appVersion: '1.0.0',
		feedbackEmail: 'dev@software-by-design.de',
		githubIssuesUrl: 'https://github.com/aSoftwareByDesignRepository/nextcloud-projectcheck/issues',
		cssPrefix: 'pc',
		...(extra || {}),
	};
	const cfgEl = {
	...
	nodes.set('pc-app-feedback-config', cfgEl);
	...
	nodes.set('pc-feedback-problem', problem);
	nodes.set('pc-feedback-idea', idea);

	const trigger = {
		className: 'pc-nav-footer__trigger',
		getAttribute(k) {
			return this[k] || null;
		},
		setAttribute(k, v) {
	...
	trigger['aria-controls'] = 'pc-feedback-menu';
	const menu = {
		id: 'pc-feedback-menu',
		hidden: true,
		contains() {
	...
	nodes.set('pc-feedback-menu', menu);
	doc.querySelector = (sel) => {
		if (sel === '.pc-nav-footer__trigger') {
			return trigger;
		}
		return null;
	};
	doc.getElementById = (id) => {
		if (id === 'pc-feedback-menu') {
			return menu;
		}
		return nodes.get(id) || null;
	};
	...
	const sandbox = {
		...
		location: {
			...
			pathname: '/apps/projectcheck/today',
			search: '?token=secret&keep=1',
			href: 'https://cloud.example/apps/projectcheck/today?token=secret&keep=1',
		},
...
function main() {
	...
	const clean = sandbox.SbdAppFeedback.sanitizePageUrl(
		'https://cloud.example/apps/projectcheck/x?token=abc&keep=1&password=no'
	);
	...
	const upgradeAck = sandbox.SbdAppFeedback.sanitizePageUrl(
		'/apps/projectcheck/?IKnowThatThisIsABigInstanceAndTheUpdateRequestCouldRunIntoATimeoutAndHowToRestoreABackup=IAmSuperSureToDoThis'
	);
	assert(upgradeAck === '/apps/projectcheck/', 'strips Nextcloud web-upgrade ack query key');
	...
	process.stdout.write('app-feedback.test.js OK (projectcheck)\n');
}
